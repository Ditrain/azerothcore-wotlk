...
The reproducible native PlayerBots core, module, extraction tools, MySQL 8.0 runtime, private configuration, WoW 3.3.5a server-side client data, all four database schemas, default PlayerBots population, logical backups, LAN login path, manual native systemd workflow, Transmog increment, and Auction Bot Plus increment are installed and verified for a trusted two-to-three-player household deployment. The owner account `DITRAIN` remains the permanent owner GM at security level 2 (`SEC_GAMEMASTER`), not administrator level. Transmog apply, relog persistence, removal, and original-appearance restoration passed with collection, retroactive collection, and Plus disabled. Auction Bot Plus is pinned, restore-backed, built with PlayerBots and Transmog, configured with four ordinary dual-role market characters, populated to 800 listings in each auction house, and validated for profession-heavy seller inventory and live buyer bidding on a player auction. Neither game server is currently running; boot startup remains disabled, ports 3724 and 8085 are closed, and the private updater temporary directory is empty.
...
- Installed configuration templates: `env/dist/etc/{authserver,worldserver}.conf.dist` and `env/dist/etc/modules/{playerbots,transmog,mod_ahbot}.conf.dist`
...
- Runtime files: `env/dist/etc/authserver.conf`, `env/dist/etc/worldserver.conf`, and `env/dist/etc/modules/{playerbots,transmog,mod_ahbot}.conf`
- All five runtime configuration files are ignored by Git, owned by `ditrain`, and mode `600`. The core and PlayerBots files contain secrets and must never be read or fingerprinted remotely; the Transmog and Auction Bot Plus files are non-secret. Installed `.conf.dist` templates remain public and unchanged except when refreshed by the pinned build installation.
...
- Reconfirm MySQL is healthy and bound only to loopback, neither game server is running, `/mnt/data/wow-server/runtime/tmp` is empty and mode `700`, and all five runtime configuration files remain owned by `ditrain` and mode `600`. Treat `transmog.conf` and `mod_ahbot.conf` as non-secret; do not read or fingerprint the secret-bearing core or PlayerBots configurations.
...
- Preserve `/mnt/data/wow-server/backups/initial-playerbots-20260716T142210Z`, `/mnt/data/wow-server/backups/pre-transmog-20260718T155801Z`, and `/mnt/data/wow-server/backups/pre-ahbot-20260719T005718Z`. Before relying on or moving any bundle, rerun `sha256sum -c SHA256SUMS` and the gzip integrity checks; do not overwrite them.
...
- Treat Transmog as installed and verified at pinned commit `33ac64b2c305eb1b6fbc97310a7ecbc30c2ba4ef`; preserve its conservative runtime options and zero permanent NPC spawns. Treat Auction Bot Plus as installed and verified at pinned commit `f685832994c825f90aa5a3dc0e1620aa568e875b`; preserve its four dedicated ordinary identities, owner-only configuration, 800-per-house baseline, and slow steady-state settings. `mod-llm-chatter` remains the next uninstalled increment.
